...
class MetOfficeDataProvider(DataProvider):
    ...
    def __init__(self, window_size, batch_size=10, max_num_batches=-1,
        ...
        data= np.loadtxt(fname=data_path,dtype=float,skiprows=3)[:,2:]
        # filter out all missing datapoints and flatten to a vector
        data = data[data!=-99.99]
        # normalise data to zero mean, unit standard deviation
        mean = sum(data)/data.shape[0]
        std = np.sqrt(np.sum((data-mean)**2)/data.shape[0])
        data = (data-mean)/std
        # convert from flat sequence to windowed data
        window_data = np.zeros((data.shape[0]-window_size+1,window_size))
        for i in range(0,data.shape[0]-window_size+1):
            for j in range(0,window_size):
                window_data[i][j] = data[i+j]
        # inputs are first (window_size - 1) entries in windows
        inputs = window_data[: ,:window_size-1]
        # targets are last entry in windows
        targets = window_data[:,window_size-1]
